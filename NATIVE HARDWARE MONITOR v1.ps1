...
public class UltimateEngine {
    ...
    // V3.51: Correção de Memory Leak (Unmanaged Code)
    ...
    // V3.51: Função que encerra os Handles não-gerenciados do Windows
...
"@
    ...
    $sync.CPU_KernelData = [UltimateEngine]::GetKernelHardwareData()
    ...
    # Cronómetro de Proteção da Placa-Mãe (I2C Bus)
    $acpiTimer = [Diagnostics.Stopwatch]::StartNew()
    $forceAcpi = $true
    ...
    while ($sync.Run) {
        ...
        foreach ($g in $rawRamGroups) {
            ...
            $liveOutput += "M$($g.GrpIdx) Clock  : $($g.Speed)`nM$($g.GrpIdx) Voltage: $($g.Volts)`n"
        }
        ...
        if ($acpiTimer.ElapsedMilliseconds -ge 1000 -or $forceAcpi) {
            $cTempList = "--- ACPI SENSORS ---`n"
            $cSensors = Get-CimInstance -ClassName MSAcpi_ThermalZoneTemperature -Namespace "root/wmi" -ErrorAction SilentlyContinue
            if ($cSensors) { 
                foreach ($t in $cSensors) { 
                    $deg = [math]::Round(($t.CurrentTemperature - 2732) / 10, 1)
                    $nomeZone = $t.InstanceName -replace "ACPI\\ThermalZone\\", ""
                    $cTempList += "${nomeZone}: $deg C`n" 
                } 
            } else { $cTempList += "N/A (Admin Required)`n" }
            $sync.ACPI_Temps = $cTempList
            $acpiTimer.Restart(); $forceAcpi = $false
        }
    ...
    # V3.51: Limpeza de Memória Não-Gerenciada (Evita Memory Leaks de DLLs Nativas)
...
$fTitle = New-Object System.Drawing.Font("Consolas", 11, [System.Drawing.FontStyle]::Bold)
$fSub   = New-Object System.Drawing.Font("Consolas", 9, [System.Drawing.FontStyle]::Bold)
...
# --- COLUNA 1: CPU & SYSTEM ---
$grpCpu = New-Object System.Windows.Forms.GroupBox; $grpCpu.Location=New-Object System.Drawing.Point(20, 20); $grpCpu.Size=New-Object System.Drawing.Size(360, 860); $grpCpu.ForeColor="Yellow"; $grpCpu.Text="CPU & SYSTEM"; $grpCpu.Font=$fTitle; $form.Controls.Add($grpCpu)
...
$lAcpi    = New-Object System.Windows.Forms.Label; $lAcpi.Location=New-Object System.Drawing.Point(10, 680); $lAcpi.Size=New-Object System.Drawing.Size(340, 40); $lAcpi.Font=$fSub; $lAcpi.ForeColor="White"; $grpCpu.Controls.Add($lAcpi)
...
# --- COLUNA 2: RAM MEMORY ---
$grpRam = New-Object System.Windows.Forms.GroupBox; $grpRam.Location=New-Object System.Drawing.Point(400, 20); $grpRam.Size=New-Object System.Drawing.Size(300, 860); $grpRam.ForeColor="Yellow"; $grpRam.Text="RAM MEMORY SPECS"; $grpRam.Font=$fTitle; $form.Controls.Add($grpRam)
...
# --- COLUNA 3: NVIDIA GPU ---
$grpGpu = New-Object System.Windows.Forms.GroupBox; $grpGpu.Location=New-Object System.Drawing.Point(720, 20); $grpGpu.Size=New-Object System.Drawing.Size(340, 860); $grpGpu.ForeColor="Yellow"; $grpGpu.Text="NVIDIA GPU"; $grpGpu.Font=$fTitle; $form.Controls.Add($grpGpu)
...
$grpSys = New-Object System.Windows.Forms.GroupBox; $grpSys.Location=New-Object System.Drawing.Point(1080, 20); $grpSys.Size=New-Object System.Drawing.Size(340, 340); $grpSys.ForeColor="Yellow"; $grpSys.Text="SYSTEM & MOTHERBOARD"; $grpSys.Font=$fTitle; $form.Controls.Add($grpSys)
...
$grpSettings = New-Object System.Windows.Forms.GroupBox; $grpSettings.Location=New-Object System.Drawing.Point(1080, 370); $grpSettings.Size=New-Object System.Drawing.Size(340, 510); $grpSettings.ForeColor="Yellow"; $grpSettings.Text="SETTINGS"; $grpSettings.Font=$fTitle; $form.Controls.Add($grpSettings)
...
$cmbPrivacy.Size = New-Object System.Drawing.Size(200, 25)
...
$cmbPrivacy.Items.Add("Privacy: Disabled")
$cmbPrivacy.Items.Add("Privacy: Standard")
$cmbPrivacy.Items.Add("Privacy: Maximum")
...
$lblDisclaimer.Font = New-Object System.Drawing.Font("Consolas", 8, [System.Drawing.FontStyle]::Regular)
...
$btnApply.Add_Click({
    if (-not [string]::IsNullOrWhiteSpace($txtRefresh.Text)) {
        ...
        if ($newVal -gt 0) { # O limite continua aberto e flexível a > 0
...
$timer.Add_Tick({
    ...
    if ($syncHash.StartTrigger) {
        ...
        $sysTempDisplay = if ($syncHash.P_SysTemp -eq "N/A") { "N/A" } else { "$($syncHash.P_SysTemp) C" }

        if ($winCores.Visible) {
            if ($lCoresLive.Text -ne $syncHash.P_Cores) { $lCoresLive.Text = $syncHash.P_Cores }
        }

        $newCpuData = "Load            : $($syncHash.P_CpuLoad) %`nClock           : $($syncHash.P_CpuFreq) MHz`nBase Clock      : $($syncHash.CPU_BaseClock) MHz`nPerformance     : $($syncHash.P_CpuPerf) %`nState           : $($syncHash.P_State)`nTopology        : $($syncHash.CPU_Topo)`n$($syncHash.CPU_Cache)`nQueue           : $($syncHash.P_Queue) tasks`nMem Hard Faults : $($syncHash.P_HardF) /s`nSys Temp        : $sysTempDisplay`n`n--- KERNEL & HARDWARE ---`n$($syncHash.CPU_KernelData)`n`n--- KERNEL PULSE ---`nSystem Calls    : $($syncHash.P_SysCalls) /s`nUser Mode       : $($syncHash.P_User) %`nKernel Mode     : $($syncHash.P_Priv) %`nDPC Time        : $($syncHash.P_Dpc) %`nInterrupts      : $($syncHash.P_Int) %"
        ...
        if ($lAcpi.Text -ne $syncHash.ACPI_Temps) { 
            $lAcpi.Text = $syncHash.ACPI_Temps 
            if (-not $syncHash.IsAdmin) { $lAcpi.ForeColor = [System.Drawing.Color]::Red } 
            else { $lAcpi.ForeColor = [System.Drawing.Color]::White }
        }
